# Numerus Ltd. 
# Purpose : Functions to fit Jackson MAIC method extracted from Jackson (2021) supplementary material (Quality Control)
...
MAIC <- function (the_data, id, match_mean, agg_means, agg_sds, accuracy=sqrt(.Machine$double.eps)) {
  ...
  the_data_o$weight.maic                              <- the_data_o$weight.maic/sum(the_data_o$weight.maic) # rescaling
