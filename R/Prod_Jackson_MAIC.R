# Numerus Ltd. 
# Purpose: Define MAIC Functions to fit Jackson MAIC method extracted from Jackson (2021) supplementary material (Production Code)

# Notes:   This function computes conventional MAIC weights.
#          It also computes our new "optimal" weights.

JACK_MAIC<-function (the_data, id, match_mean, agg_means, agg_sds, accuracy=sqrt(.Machine$double.eps)) 
{
  ...
  start=rep(0, ncol(X)); Answers=JACK_weight(start, X, accuracy)
...
JACK_weight<-function(start, X, accuracy) {  
